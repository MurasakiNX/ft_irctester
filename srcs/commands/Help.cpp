...
std::string Help::execute(Client* client, std::vector<std::string> params) {
    ...
    print_colored("= [4]: /sendf_as_privmsg <destination,key> <file>: Sends a the content of a file as a PRIVMSG to the destination (If it's a channel, the key may be required)   =", CYAN);
    print_colored("= [5]: /sendf_as_raw <file>: Sends a the content of a file as a message to the server                                                                           =", CYAN);
